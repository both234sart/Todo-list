...
import { Cat, Loader2, User } from 'lucide-react';
import { supabase } from '../lib/supabase';

interface LoginProps {
  onLogin: () => void; // Kept for legacy compatibility
  onGuestLogin: () => void;
}
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all hover:scale-[1.01] duration-300">
        <div className="p-8 text-center">
          <div className="flex justify-center mb-6">
            <div className="bg-cat-orange p-4 rounded-full">
          ...
          <div className="space-y-3">
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 px-6 py-3 border border-cat-accent bg-orange-50/50 rounded-xl hover:bg-orange-100 transition-colors duration-200 group"
              >
                 <User className="w-5 h-5 text-cat-orange-dark" />
                <span className="text-cat-brown font-medium">
                    Continue as Guest
                </span>
              </button>
          </div>
